...
public:
	...
	static void* Calloc(size_t NumElements, size_t ElementSize)
	{
		...
		auto Data = FMemory_Sinum::Malloc(TotalSize);

		if (!Data)
			return NULL;

		FMemory_Sinum::Memzero(Data, TotalSize);
	...
	static char* Strdup(const char* Str)
	{
		...
		auto StrDup = (char*)FMemory_Sinum::Malloc(StrLen);

		FMemory_Sinum::Memcpy(StrDup, Str, StrLen);
