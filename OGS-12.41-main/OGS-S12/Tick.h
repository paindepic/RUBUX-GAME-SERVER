...
namespace QuestTickingService {
    void Tick(AFortGameModeAthena* GameMode, AFortGameStateAthena* GameState) {
        if (!Globals::bQuestSystemEnabled || !GameMode) return;
        ...
        for (AFortPlayerControllerAthena* Player : GameMode->AlivePlayers) {
            ...
            if (PlayerQuestDataMap.find(Player) == PlayerQuestDataMap.end()) {
                Quests::GenerateDailyQuests(Player);
            }
            
            FPlayerQuestData& Data = PlayerQuestDataMap[Player];
            
            // Update survival time quest (every minute)
            if ((int32)(CurrentTime - Data.SurvivalStartTime) % 60 == 0) {
                int32 MinutesSurvived = (int32)((CurrentTime - Data.SurvivalStartTime) / 60.0f);
                if (MinutesSurvived > 0) {
                    Quests::UpdateQuestProgress(Player, EDailyQuestType::Survival, 1);
                }
            }
            
            // Update distance traveled quest
            if (Player->Pawn) {
                FVector CurrentPos = Player->Pawn->K2_GetActorLocation();
                if (!Data.LastPosition.IsZero()) {
                    float Distance = UKismetMathLibrary::GetDefaultObj()->Vector_Distance(Data.LastPosition, CurrentPos);
                    Data.TotalDistanceTraveled += Distance;
                    
                    // Update every 1000 units
                    if (Data.TotalDistanceTraveled >= 1000.0f) {
                        Quests::UpdateQuestProgress(Player, EDailyQuestType::Distance, 1);
                        Data.TotalDistanceTraveled = 0.0f;
                    }
                }
...
namespace Tick {
    ...
    void TickFlush(UNetDriver* Driver, float DeltaTime)
    {
        ...
        if (GameState->GamePhase > EAthenaGamePhase::Warmup && !Globals::bCreativeEnabled) {
            ...
            QuestTickingService::Tick(GameMode, GameState);
        }
