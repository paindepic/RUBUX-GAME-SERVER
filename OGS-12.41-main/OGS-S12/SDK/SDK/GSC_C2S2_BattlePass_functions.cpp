#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: GSC_C2S2_BattlePass

#include "Basic.hpp"

#include "GSC_C2S2_BattlePass_classes.hpp"
#include "GSC_C2S2_BattlePass_parameters.hpp"


namespace SDK
{

// Function GSC_C2S2_BattlePass.GSC_C2S2_BattlePass_C.OnNavActorSelected
// (Event, Public, BlueprintEvent)

void UGSC_C2S2_BattlePass_C::OnNavActorSelected()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("GSC_C2S2_BattlePass_C", "OnNavActorSelected");

	UObject::ProcessEvent(Func, nullptr);
}


// Function GSC_C2S2_BattlePass.GSC_C2S2_BattlePass_C.OnNavActorHovered
// (Event, Public, BlueprintEvent)

void UGSC_C2S2_BattlePass_C::OnNavActorHovered()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("GSC_C2S2_BattlePass_C", "OnNavActorHovered");

	UObject::ProcessEvent(Func, nullptr);
}


// Function GSC_C2S2_BattlePass.GSC_C2S2_BattlePass_C.ExecuteUbergraph_GSC_C2S2_BattlePass
// (Final, UbergraphFunction)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UGSC_C2S2_BattlePass_C::ExecuteUbergraph_GSC_C2S2_BattlePass(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("GSC_C2S2_BattlePass_C", "ExecuteUbergraph_GSC_C2S2_BattlePass");

	Params::GSC_C2S2_BattlePass_C_ExecuteUbergraph_GSC_C2S2_BattlePass Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}

}

