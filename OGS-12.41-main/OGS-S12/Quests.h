...
// ===== Daily Quest System for Players =====
...
struct FDailyQuest {
    ...
    FDailyQuest() : QuestType(EDailyQuestType::Eliminations), Progress(0), Required(3), RewardXP(1000), bCompleted(false) {}
    
    FDailyQuest(EDailyQuestType Type, int32 Req, int32 XP, FString Desc)
        : QuestType(Type), Progress(0), Required(Req), RewardXP(XP), bCompleted(false), Description(Desc) {}
};
...
struct FPlayerQuestData {
    ...
    FPlayerQuestData() : LastQuestRefreshTime(0.0f), SurvivalStartTime(0.0f), TotalDistanceTraveled(0.0f) {}
};

static std::map<AFortPlayerControllerAthena*, FPlayerQuestData> PlayerQuestDataMap;

namespace Quests {
    void GenerateDailyQuests(AFortPlayerControllerAthena* PC) {
        if (!PC || !Globals::bQuestSystemEnabled) return;
        
        FPlayerQuestData& Data = PlayerQuestDataMap[PC];
        Data.DailyQuests.Clear();
        Data.LastQuestRefreshTime = UGameplayStatics::GetDefaultObj()->GetTimeSeconds(UWorld::GetWorld());
        Data.SurvivalStartTime = Data.LastQuestRefreshTime;
        
        // Quest 1: Combat quest (Eliminations)
        Data.DailyQuests.Add(FDailyQuest(EDailyQuestType::Eliminations, 3, 2000, L"Eliminate 3 players"));
        
        // Quest 2: Looting quest (Chests)
        Data.DailyQuests.Add(FDailyQuest(EDailyQuestType::Chests, 5, 1500, L"Open 5 chests"));
        
        // Quest 3: Survival quest
        Data.DailyQuests.Add(FDailyQuest(EDailyQuestType::Survival, 10, 2500, L"Survive for 10 minutes"));
        
        // Optional: Boss quest (if bosses enabled)
        if (Globals::bBossVaultSystemEnabled && Globals::bBossesEnabled) {
            Data.DailyQuests.Add(FDailyQuest(EDailyQuestType::BossKill, 1, 3000, L"Eliminate a boss"));
            Data.DailyQuests.Add(FDailyQuest(EDailyQuestType::VaultOpen, 1, 2000, L"Open a vault"));
        }
    }
    
    void UpdateQuestProgress(AFortPlayerControllerAthena* PC, EDailyQuestType QuestType, int32 Amount = 1) {
        if (!PC || !Globals::bQuestSystemEnabled) return;
        
        if (PlayerQuestDataMap.find(PC) == PlayerQuestDataMap.end()) {
            GenerateDailyQuests(PC);
        }
        
        FPlayerQuestData& Data = PlayerQuestDataMap[PC];
        
        for (FDailyQuest& Quest : Data.DailyQuests) {
            if (Quest.QuestType == QuestType && !Quest.bCompleted) {
                ...
                if (Quest.Progress >= Quest.Required) {
                    ...
                    // Grant XP reward
                    if (PC->XPComponent) {
                        PC->XPComponent->ChallengeXp += Quest.RewardXP;
                        PC->XPComponent->TotalXpEarned += Quest.RewardXP;
                        PC->XPComponent->MatchXp += Quest.RewardXP;
                        PC->XPComponent->OnXpUpdated(PC->XPComponent->CombatXp, PC->XPComponent->SurvivalXp, 
                            PC->XPComponent->MedalBonusXP, PC->XPComponent->ChallengeXp, 
